...
public:
	...
	CSvmTrainer(KernelType* kernel, double C, bool offset, bool unconstrained = false, bool computeDerivative = true, bool warmRestart = false)
	: base_type(kernel, C, offset, unconstrained)
    , m_computeDerivative(computeDerivative)
    , m_warmRestart (warmRestart)
	{ }
	...
	//! \param offset whether to train the svm with offset term
	//! \param  unconstrained  when a C-value is given via setParameter, should it be piped through the exp-function before using it in the solver?
	CSvmTrainer(KernelType* kernel, double negativeC, double positiveC, bool offset, bool unconstrained = false, bool warmRestart = false)
	: base_type(kernel,negativeC, positiveC, offset, unconstrained)
...
private:
    ...
    // if this is true, the 'setStructure' of the kernel decisionFunction will not be called
    bool m_warmRestart;
