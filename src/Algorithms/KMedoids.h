 ...
 *
 *
 * \brief       The k-medoids clustering algorithm.
 *
 *
 *
 * \author      Aydin Demircioglu, T. Glasmachers
 * \date        2014
 *
 *
...
#ifndef SHARK_ALGORITHMS_KMEDOIDS_H
#define SHARK_ALGORITHMS_KMEDOIDS_H
...
///
    std::size_t kMedoids (Data<RealVector> const& data, std::size_t k, Centroids& centroids, std::size_t maxIterations = 0);

    template<class InputType>
    KernelExpansion<InputType> kMedoids (Data<InputType> const& dataset, std::size_t k, AbstractKernelFunction<InputType>& kernel, std::size_t maxIterations = 0) {
        if (!maxIterations)
