 ...
 *
 *
 * \brief       SVMperf clone. Hopefully better working one.
 *
 *
 *
 *
...
#ifndef SHARK_ALGORITHMS_KERNELCPATRAINER_H
#define SHARK_ALGORITHMS_KERNELCPATRAINER_H
...
///
    ...
    class KernelCPATrainer : public AbstractTrainer< KernelClassifier<InputType> >, public IParameterizable {
        public:
            ...
            KernelCPATrainer (KernelType *kernel, const LossType *loss, double C, bool offset, bool unconstrained = false)
                : m_kernel (kernel)
                , m_loss (loss)
                , m_C (C)
//        , m_epsilon(epsilon)
            ...
            void train (ClassifierType &classifier, const LabeledData<InputType, unsigned int> &dataset) {
                ...
                for (std::size_t iter = 0; iter < iterations; iter++) {
                    ...
                    // solve by using CSVM trainer
                    CSvmTrainer<RealVector> *kernelCSVMTrainer = new CSvmTrainer<RealVector> (kernel, m_C, m_offset);
                    kernelCSVMTrainer->stoppingCondition().minAccuracy = epsilon;
                    kernelCSVMTrainer->setCacheSize (cacheSize);

                    // get alphas

                    // compute current slack variable
                    ...
                    // check if the most violated constraint
                    // is violated more than epsilon
                }
